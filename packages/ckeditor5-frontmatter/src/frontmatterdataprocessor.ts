...
export default class FrontmatterDataProcessor implements DataProcessor {
	...
	public keepHtml( element: keyof HTMLElementTagNameMap ): void {
		const markdownProcessor = this._markdownProcessor as HtmlKeepingDataProcessor;
...
function parseFrontmatter( data: string ): FrontmatterParts {
	...
	const closingLineIndex = lines.findIndex( ( line, index ) => (
		index > 0 && line === '---'
	) );
...
function extractFrontmatter( viewFragment: ViewDocumentFragment ): {
	...
	const frontmatterElement = Array
		.from( firstChild.getChildren() )
		.find( isFrontmatterElement );
...
function isFrontmatterContainer( node: ViewNode | undefined ): node is ViewElement {
	return !!node && node.is( 'element', 'section' ) && node.hasClass( 'frontmatter-container' );
}

function isFrontmatterElement( node: ViewNode ): node is ViewElement {
	return node.is( 'element', 'frontmatter' ) ||
