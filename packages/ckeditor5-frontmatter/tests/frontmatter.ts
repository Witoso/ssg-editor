...
describe( 'Frontmatter', () => {
	...
	describe( 'init()', () => {
		...
		it( 'should add an icon to the toolbar', () => {
			expect( editor.ui.componentFactory.has( 'frontmatter' ) ).toBe(
				true
			);
		} );
		...
		it( 'should wrap the final Markdown data processor', async () => {
			class CustomMarkdownProcessorPlugin extends Plugin {
				...
				public afterInit(): void {
					...
					this.editor.data.processor = {
						toView: data => processor.toView( data.replace( '{{custom}}', '## Custom' ) ),
						toData: viewFragment => processor.toData( viewFragment ),
						registerRawContentMatcher: pattern => processor.registerRawContentMatcher( pattern ),
			...
			expect( ( processor as FrontmatterDataProcessor ).innerProcessor ).not.toBeInstanceOf(
	...
	describe( 'schema', () => {
		...
		it( 'should only allow the frontmatter container in the document root', () => {
			...
			expect(
				schema.checkChild( [ '$root' ], 'frontmatterContainer' )
			).toBe( true );
			expect(
	...
	describe( 'collapsible', () => {
		...
		it( 'should mark the frontmatter container as collapsible', () => {
			...
			expect(
				getViewContainer().hasClass( 'frontmatter-collapsible' )
			).toBe( true );
		} );
		...
		it( 'should mark the container while the selection is inside the focused frontmatter', () => {
			...
			expect(
				getViewContainer().hasClass( 'frontmatter-selected' )
			).toBe( false );
